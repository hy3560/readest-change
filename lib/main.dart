...
class _ReadestXState extends State<ReadestX> {
  ...
  Future<void> _importBook() async {
    FilePickerResult? result = await FilePicker.platform.pickFiles(
      ...
      allowedExtensions: ['pdf'], // 现在只支持极其稳定的 PDF
    );
    if (result != null) {
  ...
  void _readBook(PlatformFile file) {
    if (file.extension == 'pdf') {
      Navigator.push(context, MaterialPageRoute(builder: (_) => Scaffold(
        appBar: AppBar(title: Text(file.name), backgroundColor: Colors.black),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("READEST LITE", style: TextStyle(fontWeight: FontWeight.bold, letterSpacing: 1.5)),
      ...
      body: Column(
        children: [
          ...
          Expanded(child: _myBooks.isEmpty ? _buildEmptyHome() : _buildBookGrid()),
        ],
      ),
        ...
        children: [
          TextField(
            ...
            decoration: InputDecoration(
              hintText: "VPS 端点 (例: http://ip:8000/v1)",
          ...
          DropdownButton<String>(
            ...
            hint: Text(_isConnecting ? "正在连接..." : "获取音色列表", style: const TextStyle(color: Colors.white30)),
  ...
  Widget _buildBookGrid() {
    return GridView.builder(
      ...
      itemBuilder: (ctx, i) => GestureDetector(
        ...
        child: Column(
          children: [
            Expanded(child: Container(decoration: BoxDecoration(color: Colors.white10, borderRadius: BorderRadius.circular(10)), child: const Icon(Icons.picture_as_pdf, color: Colors.redAccent, size: 40))),
  ...
  Widget _buildEmptyHome() {
    return const Center(
      child: Opacity(opacity: 0.3, child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        SizedBox(height: 10),
        Text("点击右下角导入 PDF", style: TextStyle(color: Colors.white, fontSize: 16)),
      ])),
    );
  }
}